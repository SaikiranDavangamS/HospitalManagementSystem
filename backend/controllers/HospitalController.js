...
const addHospital = async (req, res) => {
  try {
    ...
    const { hospital_name, hospital_address, hospital_phone, hospital_email } =
      req.body;
    ...
    if (existingHospital) {
      return res
        .status(400)
        .json({ message: "Hospital Details already exists" });
    } else {
...
const updateHospital = async (req, res) => {
  try {
    const { hospital_name, hospital_email, hospital_phone, hospital_address } =
      req.body;
...
const countAllHospitals = async (req, res) => {
  try {
    ...
    if (!allhospitals)
      return res
        .status(404)
        .json({ message: "Failed to fetch Hospital count" });
    res.status(200).json({ allhospitals });
  } catch (err) {
    console.error("Fetch Error:", err.message);
    res.status(500).json({ message: "Server error" });
